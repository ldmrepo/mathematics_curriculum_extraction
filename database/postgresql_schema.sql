...
-- 생성일: 2024-12-19
-- 버전: 1.0.0
-- 설명: 한국 수학과 교육과정 데이터를 위한 PostgreSQL 스키마
...
CREATE TABLE achievement_standards (
    ...
    standard_title VARCHAR(100) NOT NULL,
    ...
    CONSTRAINT chk_standard_code_format CHECK (standard_code ~ '^[0-9]{1}수[0-9]{2}-[0-9]{2}$')
);
...
CREATE TABLE standard_explanations (
    ...
    explanation_type VARCHAR(20) NOT NULL CHECK (explanation_type IN ('성취기준 해설', '적용시 고려사항')),
...
USING gin(to_tsvector('korean', standard_content));

CREATE INDEX idx_terms_symbols_name_search ON terms_symbols 
USING gin(to_tsvector('korean', term_name || ' ' || COALESCE(term_description, '')));
...
BEGIN
    ...
    SELECT 
        ...
        ast.standard_title,
        ast.standard_content,
        ts_rank(to_tsvector('korean', ast.standard_content), plainto_tsquery('korean', search_term)) as rank
    FROM achievement_standards ast
        JOIN school_levels sl ON ast.level_id = sl.level_id
        JOIN domains d ON ast.domain_id = d.domain_id
    WHERE to_tsvector('korean', ast.standard_content) @@ plainto_tsquery('korean', search_term)
    ORDER BY rank DESC;
END;
$$ LANGUAGE plpgsql;
...
COMMENT ON SCHEMA curriculum IS '한국 수학과 교육과정 데이터베이스 스키마';
...
COMMENT ON TABLE standard_explanations IS '성취기준 해설 및 적용시 고려사항';
COMMENT ON TABLE terms_symbols IS '수학 용어 및 기호 (LaTeX 표현 포함)';

-- 중요 컬럼에 대한 코멘트
COMMENT ON COLUMN achievement_standards.standard_code IS '성취기준 코드 (예: 2수01-01)';
...
SELECT 
    ...
    ROUND(
        COUNT(CASE WHEN latex_expression IS NOT NULL THEN 1 END) * 100.0 / COUNT(*), 
        2
    ) as latex_percentage
FROM terms_symbols;
*/
