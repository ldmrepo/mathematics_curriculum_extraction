...
-- 수정일: 2025-01-21
-- 버전: 1.1.0
-- 설명: 2022 개정 수학과 교육과정 데이터를 위한 PostgreSQL 스키마
...
CREATE TABLE content_elements (
    ...
    element_name VARCHAR(200) NOT NULL,  -- 크기 증가 (100 -> 200)
...
CREATE TABLE achievement_standards (
    ...
    standard_title VARCHAR(200) NOT NULL,  -- 크기 증가 (100 -> 200)
    ...
    CONSTRAINT chk_standard_code_format CHECK (standard_code ~ '^[0-9]{1,2}수[0-9]{2}-[0-9]{2}$')  -- 수정: 1-2자리 숫자 허용
);

-- 3.2 성취기준 해설
CREATE TABLE standard_explanations (
    explanation_id SERIAL PRIMARY KEY,
    standard_id INTEGER REFERENCES achievement_standards(standard_id) ON DELETE CASCADE,  -- NOT NULL 제거 (일반 해설용)
    explanation_type VARCHAR(30) NOT NULL CHECK (explanation_type IN ('성취기준 해설', '적용시 고려사항', '용어와 기호')),  -- '용어와 기호' 추가
...
CREATE TABLE terms_symbols (
    ...
    latex_expression TEXT, -- LaTeX 표현이 있는 경우
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
-- 전문 검색을 위한 인덱스 (PostgreSQL의 전문 검색 기능 활용)
CREATE INDEX idx_achievement_standards_content_search ON achievement_standards 
USING gin(to_tsvector('simple', standard_content));  -- 'korean' -> 'simple' (설정 독립적)
...
-- 6.4 성취기준 해설 통합 뷰 (추가)
CREATE VIEW v_standard_explanations_full AS
SELECT 
    se.explanation_id,
    ast.standard_code,
    sl.grade_range,
    ...
    se.explanation_order
FROM standard_explanations se
    LEFT JOIN achievement_standards ast ON se.standard_id = ast.standard_id
    LEFT JOIN school_levels sl ON ast.level_id = sl.level_id
    LEFT JOIN domains d ON ast.domain_id = d.domain_id
ORDER BY ast.standard_code, se.explanation_type, se.explanation_order;
...
-- 7.3 용어 검색 함수 (추가)
CREATE OR REPLACE FUNCTION search_terms_symbols(search_term TEXT)
RETURNS TABLE (
    term_id INTEGER,
    school_type VARCHAR(20),
    grade_range VARCHAR(20),
...
BEGIN
    ...
    SELECT 
        ...
        ts.term_name,
        ts.term_description,
        ts_rank(to_tsvector('simple', ts.term_name || ' ' || COALESCE(ts.term_description, '')), 
                plainto_tsquery('simple', search_term)) as rank
    FROM terms_symbols ts
        JOIN school_levels sl ON ts.level_id = sl.level_id
        JOIN domains d ON ts.domain_id = d.domain_id
    WHERE to_tsvector('simple', ts.term_name || ' ' || COALESCE(ts.term_description, '')) 
...
COMMENT ON SCHEMA curriculum IS '2022 개정 수학과 교육과정 데이터베이스 스키마';
...
COMMENT ON COLUMN school_levels.level_code IS '성취기준 코드에 사용되는 학년 코드';
COMMENT ON COLUMN standard_explanations.explanation_type IS '해설 유형: 성취기준 해설, 적용시 고려사항, 용어와 기호';
...
FROM terms_symbols ts
    JOIN school_levels sl ON ts.level_id = sl.level_id
    JOIN domains d ON ts.domain_id = d.domain_id
GROUP BY sl.level_id, sl.grade_range, d.domain_id, d.domain_name, ts.term_type
ORDER BY sl.level_id, d.domain_order, ts.term_type;
*/
